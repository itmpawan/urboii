...
class Organey_Admin_MegaMenu {
    ...
    private function includes() {
        include_once get_template_directory() . '/includes/megamenu/includes/admin/class-admin-assets.php';
    }

    public function add_page_templates($page_templates, $wp_theme, $post) {
        $page_templates['includes/megamenu/templates/single-menu.php'] = 'Single Mega Menu';
        return $page_templates;
    }

    public function process_create_related($post_id) {
        if (isset($_GET['organey-menu-createable']) && isset($_GET['menu_id']) && absint($_GET['menu_id'])) {
            ...
            if ($related_id && isset($_REQUEST['menu_id']) && is_admin()) {
                ...
                $action = add_query_arg(array('organey-menu-editable' => 1), $url);
