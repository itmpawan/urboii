...
class Merlin {
	...
	private function version() {
		...
		if ( ! defined( 'MERLIN_VERSION' ) ) {
			define( 'MERLIN_VERSION', ORGANEY_VERSION );
		}
	}
...
//		add_action( 'after_switch_theme', array( $this, 'switch_theme' ) );
	...
	public function admin_page() {
		...
		// Enqueue javascript.
		wp_enqueue_script( 'merlin', $this->base_url . $this->directory . '/assets/js/merlin.js', array( 'jquery-core' ), MERLIN_VERSION );

		$texts = array(
			'something_went_wrong' => esc_html__( 'Something went wrong. Please refresh the page and try again!', 'organey' ),
		);
	...
	public function svg( $args = array() ) {
		...
		if ( empty( $args ) ) {
			return esc_html__( 'Please define default parameters in the form of an array.', 'organey' );
		}

		// Define an icon.
		if ( false === array_key_exists( 'icon', $args ) ) {
			return esc_html__( 'Please define an SVG icon filename.', 'organey' );
		}
	...
	public function steps() {
		...
		$this->steps = array(
			'welcome' => array(
				'name'    => esc_html__( 'Welcome', 'organey' ),
		...
		if ( class_exists( 'TGM_Plugin_Activation' ) ) {
			$this->steps['plugins'] = array(
				'name' => esc_html__( 'Plugins', 'organey' ),
		...
		if ( ! empty( $this->import_files ) ) {
			$this->steps['content'] = array(
				'name' => esc_html__( 'Content', 'organey' ),
				'view' => array( $this, 'content' ),
			);
		}

		$this->steps['ready'] = array(
			'name' => esc_html__( 'Ready', 'organey' ),
	...
	protected function welcome() {
		...
		<?php
		$this->logger->debug( esc_html__( 'The welcome step has been displayed', 'organey' ) );
	}
	...
	protected function license() {
		...
		<?php
		$this->logger->debug( esc_html__( 'The license activation step has been displayed', 'organey' ) );
	}
	...
	protected function child() {
		...
		<?php
		$this->logger->debug( esc_html__( 'The child theme installation step has been displayed', 'organey' ) );
	}
	...
	protected function plugins() {
						...
						<?php foreach ( $required_plugins as $slug => $plugin ) : ?>
                            <li data-slug="<?php echo esc_attr( $slug ); ?>">
                                ...
                                <label for="default_plugins_<?php echo esc_attr( $slug ); ?>">
                                    ...
                                    <span class="badge">
									<span class="hint--top" aria-label="<?php esc_attr_e( 'Required', 'organey' ); ?>">
										<?php esc_html_e( 'req', 'organey' ); ?>
									</span>
								</span>
                                </label>
                            </li>
		...
		<?php
		$this->logger->debug( esc_html__( 'The plugin installation step has been displayed', 'organey' ) );
	}
	...
	protected function content() {
			...
			<?php if ( 1 < count( $this->import_files ) ) : ?>
                ...
                <div class="merlin__select-control-wrapper">
                    ...
                    <div class="merlin__select-control-help">
						<span class="hint--top" aria-label="<?php echo esc_attr__( 'Select Demo', 'organey' ); ?>">
		...
		<?php
		$this->logger->debug( esc_html__( 'The content import step has been displayed', 'organey' ) );
	}
	...
	protected function ready() {
		...
		<?php
		$this->logger->debug( esc_html__( 'The final step has been displayed', 'organey' ) );
	}
	...
	public function generate_child() {
		...
		if ( ! file_exists( $path ) ) {
			...
			$this->logger->debug( esc_html__( 'The existing child theme was activated', 'organey' ) );
		...
		$this->logger->debug( esc_html__( 'The newly generated child theme was activated', 'organey' ) );
	...
	public function _ajax_activate_license() {
		...
		if ( ! check_ajax_referer( 'merlin_nonce', 'wpnonce' ) ) {
			wp_send_json(
				array(
					'success' => false,
					'message' => esc_html__( 'Yikes! The theme activation failed. Please try again or contact support.', 'organey' ),
				)
			);
		}
		...
		if ( empty( $_POST['license_key'] ) ) {
			wp_send_json(
				array(
					'success' => false,
					'message' => esc_html__( 'Please add your license key before attempting to activate one.', 'organey' ),
				)
			);
		}
		...
		$this->logger->debug( esc_html__( 'The license activation was performed with the following results', 'organey' ), $result );
	...
	protected function edd_activate_license( $license ) {
		...
		if ( is_wp_error( $response ) || 200 !== wp_remote_retrieve_response_code( $response ) ) {
			...
			if ( is_wp_error( $response ) ) {
				...
				$message = esc_html__( 'An error occurred, please try again.', 'organey' );
			}
		} else {
				...
				switch ( $license_data->error ) {
					...
					case 'expired':
						...
						/* translators: Expiration date */
							esc_html__( 'Your license key expired on %s.', 'organey' ),
							date_i18n( get_option( 'date_format' ), strtotime( $license_data->expires, current_time( 'timestamp' ) ) )
						);
						break;

					case 'revoked':
						$message = esc_html__( 'Your license key has been disabled.', 'organey' );
						break;

					case 'missing':
						$message = esc_html__( 'This appears to be an invalid license key. Please try again or contact support.', 'organey' );
						break;

					case 'invalid':
					case 'site_inactive':
						$message = esc_html__( 'Your license is not active for this URL.', 'organey' );
						break;

					case 'item_name_mismatch':
						/* translators: EDD Item Name */
						$message = sprintf( esc_html__( 'This appears to be an invalid license key for %s.', 'organey' ), $this->edd_item_name );
						break;

					case 'no_activations_left':
						$message = esc_html__( 'Your license key has reached its activation limit.', 'organey' );
						break;
	...
	public function generate_child_functions_php( $slug ) {
		...
		$this->logger->debug( esc_html__( 'The child theme functions.php content was generated', 'organey' ) );
	...
	public function generate_child_style_css( $slug, $parent, $author, $version ) {
		...
		$this->logger->debug( esc_html__( 'The child theme style.css content was generated', 'organey' ) );
	...
	public function generate_child_screenshot( $path ) {
		...
		if ( ! empty( $screenshot ) && file_exists( $screenshot ) ) {
			...
			$this->logger->debug( esc_html__( 'The child theme screenshot was copied to the child theme, with the following result', 'organey' ), array( 'copied' => $copied ) );
		} else {
			$this->logger->debug( esc_html__( 'The child theme screenshot was not generated, because of these results', 'organey' ), array( 'screenshot' => $screenshot ) );
		}
	}
	...
	function _ajax_plugins() {
		...
		foreach ( $plugins['activate'] as $slug => $plugin ) {
			if ( $_POST['slug'] === $slug ) {
				$json = array(
					...
					'action2'       => - 1,
					'message'       => esc_html__( 'Activating', 'organey' ),
				);
				break;
			}
		}
		...
		foreach ( $plugins['update'] as $slug => $plugin ) {
			if ( $_POST['slug'] === $slug ) {
				$json = array(
					...
					'action2'       => - 1,
					'message'       => esc_html__( 'Updating', 'organey' ),
				);
				break;
			}
		}
		...
		foreach ( $plugins['install'] as $slug => $plugin ) {
			if ( $_POST['slug'] === $slug ) {
				$json = array(
					...
					'action2'       => - 1,
					'message'       => esc_html__( 'Installing', 'organey' ),
				);
				break;
			}
		}
		...
		if ( $json ) {
			$this->logger->debug(
				esc_html__( 'A plugin with the following data will be processed', 'organey' ),
				array(
			...
			$json['message'] = esc_html__( 'Installing', 'organey' );
			wp_send_json( $json );
		} else {
			$this->logger->debug(
				esc_html__( 'A plugin with the following data was processed', 'organey' ),
				array(
					'plugin_slug' => $_POST['slug'],
				)
			);
			...
			wp_send_json(
				array(
					'done'    => 1,
					'message' => esc_html__( 'Success', 'organey' ),
				)
			);
		}
	...
	function _ajax_content() {
		...
		if ( ! check_ajax_referer( 'merlin_nonce', 'wpnonce' ) || empty( $_POST['content'] ) && isset( $content[ $_POST['content'] ] ) ) {
			$this->logger->error( esc_html__( 'The content importer AJAX call failed to start, because of incorrect data', 'organey' ) );

			wp_send_json_error(
				array(
					'error'   => 1,
					'message' => esc_html__( 'Invalid content!', 'organey' ),
				)
			);
		}
		...
		if ( isset( $_POST['proceed'] ) ) {
			if ( is_callable( $this_content['install_callback'] ) ) {
				$this->logger->info(
					esc_html__( 'The content import AJAX call will be executed with this import data', 'organey' ),
					array(
		...
		if ( $json ) {
			...
			$this->logger->error(
				esc_html__( 'The content import AJAX call failed with this passed data', 'organey' ),
				array(
			...
			wp_send_json(
				array(
					'error'   => 1,
					'message' => esc_html__( 'Error', 'organey' ),
					'logs'    => '',
					'errors'  => '',
				)
			);
	...
	public function _ajax_get_total_content_import_items() {
		if ( ! check_ajax_referer( 'merlin_nonce', 'wpnonce' ) && empty( $_POST['selected_index'] ) ) {
			$this->logger->error( esc_html__( 'The content importer AJAX call for retrieving total content import items failed to start, because of incorrect data.', 'organey' ) );

			wp_send_json_error(
				array(
					'error'   => 1,
					'message' => esc_html__( 'Invalid data!', 'organey' ),
				)
			);
		}
	...
	public function get_import_data_info( $selected_import_index = 0 ) {
		$import_data = apply_filters( 'organey_custom_import_data_info', array(
			'content'       => false,
			'homepage'      => false,
			'blog'          => true,
			'woocommerce'   => true,
	...
	protected function get_import_data( $selected_import_index = 0 ) {
		...
		if ( ! empty( $import_files['content'] ) ) {
			$content['content'] = array(
				'title'            => esc_html__( 'Content', 'organey' ),
				'description'      => esc_html__( 'Demo content data.', 'organey' ),
		...
		if ( ! empty( $import_files['widgets'] ) ) {
			$content['widgets'] = array(
				'title'            => esc_html__( 'Widgets', 'organey' ),
				'description'      => esc_html__( 'Sample widgets data.', 'organey' ),
				'pending'          => esc_html__( 'Pending', 'organey' ),
				'installing'       => esc_html__( 'Installing', 'organey' ),
				'success'          => esc_html__( 'Success', 'organey' ),
		...
		if ( ! empty( $import_files['sliders'] ) ) {
			$content['sliders'] = array(
				'title'            => esc_html__( 'Revolution Slider', 'organey' ),
				'description'      => esc_html__( 'Sample Revolution sliders data.', 'organey' ),
				'pending'          => esc_html__( 'Pending', 'organey' ),
				'installing'       => esc_html__( 'Installing', 'organey' ),
				'success'          => esc_html__( 'Success', 'organey' ),
		...
		if ( ! empty( $import_files['homepage'] ) ) {
			$content['homepage'] = array(
				'title'            => esc_html__( 'Home Page', 'organey' ),
				'description'      => esc_html__( 'Install homepage xml.', 'organey' ),
				'pending'          => esc_html__( 'Pending', 'organey' ),
				'installing'       => esc_html__( 'Installing', 'organey' ),
				'success'          => esc_html__( 'Success', 'organey' ),
				'install_callback' => array( $this->importer, 'import' ),
		...
		if ( ! empty( $import_files['redux'] ) ) {
			$content['redux'] = array(
				'title'            => esc_html__( 'Redux Options', 'organey' ),
				'description'      => esc_html__( 'Redux framework options.', 'organey' ),
				'pending'          => esc_html__( 'Pending', 'organey' ),
				'installing'       => esc_html__( 'Installing', 'organey' ),
				'success'          => esc_html__( 'Success', 'organey' ),
		...
		if ( false !== has_action( 'merlin_after_all_import' ) ) {
			$content['after_import'] = array(
				'title'            => esc_html__( 'After import setup', 'organey' ),
				'description'      => esc_html__( 'After import setup.', 'organey' ),
				'pending'          => esc_html__( 'Pending', 'organey' ),
				'installing'       => esc_html__( 'Installing', 'organey' ),
				'success'          => esc_html__( 'Success', 'organey' ),
		...
		$content = apply_filters( 'merlin_get_base_content', $content, $this, $import_files );
	...
	public function import_revolution_sliders( $file ) {
		...
		$this->logger->info( esc_html__( 'The revolution slider import was executed', 'organey' ) );
	...
	public function pt_importer_new_ajax_request_response_data( $data ) {
		...
		$data['message']  = esc_html__( 'Installing', 'organey' );
		$data['proceed']  = 'true';
	...
	public function after_content_import_setup() {
		...
		if ( $homepage ) {
			...
			$this->logger->debug( esc_html__( 'The home page was set', 'organey' ), array( 'homepage_id' => $homepage ) );
		}
		...
		if ( $blogpage ) {
			...
			$this->logger->debug( esc_html__( 'The blog page was set', 'organey' ), array( 'blog_page_id' => $blogpage ) );
		}
	}
	...
	public function before_content_import_setup() {
		...
		if ( ! empty( $hello_world ) ) {
			...
			$this->logger->debug( esc_html__( 'The Hello world post status was set to draft', 'organey' ) );
		}
	}
	...
	public function validate_import_file_info( $import_files ) {
		...
		foreach ( $import_files as $import_file ) {
			if ( ! empty( $import_file['import_file_name'] ) ) {
				...
				$this->logger->warning( esc_html__( 'This predefined demo import does not have the name parameter: import_file_name', 'organey' ), $import_file );
			}
		}
	...
	public function get_import_files_paths( $selected_import_index ) {
		...
		return apply_filters( 'organey_get_import_files_paths', $import_files );
	}
	...
	public function get_import_steps_html( $import_info ) {
		$list_title = apply_filters( 'organey_get_import_steps_text', array(
			'content'       => esc_html__( 'All Page + Image', 'organey' ),
			'homepage'      => esc_html__( 'Home Page', 'organey' ),
			'blog'          => esc_html__( 'Blog', 'organey' ),
			'woocommerce'   => esc_html__( 'Products', 'organey' ),
			'header-footer' => esc_html__( 'Header + Footer + Breadcrumb', 'organey' ),
			'widgets'       => false,
			'options'       => false,
			'sliders'       => false,
			'redux'         => false,
			'after_import'  => esc_html__( 'Setup Theme', 'organey' ),
		) );
		ob_start();
		?>
		<?php foreach ( $import_info as $slug => $available ) : ?>
            ...
            <li class="merlin__drawer--import-content__list-item status status--Pending" data-content="<?php echo esc_attr( $slug ); ?>">
                ...
                <label for="default_content_<?php echo esc_attr( $slug ); ?>">
                    <i></i><span><?php echo $list_title[ $slug ] ? $list_title[ $slug ] : esc_html( ucfirst( str_replace( '_', ' ', $slug ) ) ); ?></span>
                </label>
            </li>
