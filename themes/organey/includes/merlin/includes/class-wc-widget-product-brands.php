 ...
 * @extends WC_Widget
 */
class Organey_Widget_Product_Brands extends WC_Widget {
	...
	public function __construct() {
		...
		$this->widget_description = esc_html__('A list or dropdown of product brands.', 'organey');
		$this->widget_id          = 'woocommerce_product_brands';
		$this->widget_name        = esc_html__('Product Brands', 'organey');
		$this->settings           = array(
			'title'              => array(
				'type'  => 'text',
				'std'   => esc_html__('Product Brands', 'organey'),
				'label' => esc_html__('Title', 'organey'),
			),
			'dropdown'           => array(
				'type'  => 'checkbox',
				'std'   => 0,
				'label' => esc_html__('Show as dropdown', 'organey'),
			),
			'show_logo'          => array(
				'type'  => 'checkbox',
				'std'   => 0,
				'label' => esc_html__('Show logo brand', 'organey'),
			),
			'count'              => array(
				'type'  => 'checkbox',
				'std'   => 0,
				'label' => esc_html__('Show product counts', 'organey'),
			),
			'hierarchical'       => array(
				'type'  => 'checkbox',
				'std'   => 1,
				'label' => esc_html__('Show hierarchy', 'organey'),
			),
			'show_children_only' => array(
				'type'  => 'checkbox',
				'std'   => 0,
				'label' => esc_html__('Only show children of the current brand', 'organey'),
			),
			'hide_empty'         => array(
				'type'  => 'checkbox',
				'std'   => 0,
				'label' => esc_html__('Hide empty brands', 'organey'),
			),
			'max_depth'          => array(
				'type'  => 'text',
				'std'   => '',
				'label' => esc_html__('Maximum depth', 'organey'),
			),
		);
	...
	public function widget($args, $instance) {
		...
		if ($dropdown) {
			...
			include_once(get_theme_file_path('includes/woocommerce/class-product-brand-list-walker.php'));

			$list_args['walker']                  = new Organey_Product_Brand_List_Walker;
			$list_args['title_li']                = '';
			$list_args['pad_counts']              = 1;
			$list_args['show_option_none']        = esc_html__('No product brands exist.', 'organey');
			...
			echo '<ul class="product-brands" id="organey-brands-' . $id . '">';
...
add_action('widgets_init', function () {
	register_widget('Organey_Widget_Product_Brands');
});

add_action('init', function () {
	$labels = array(
		'name'                       => esc_html__('Brands', 'organey'),
		'singular_name'              => esc_html__('Brands', 'organey'),
		'menu_name'                  => esc_html__('Brands', 'organey'),
		'all_items'                  => esc_html__('All Brands', 'organey'),
		'parent_item'                => esc_html__('Parent Brand', 'organey'),
		'parent_item_colon'          => esc_html__('Parent Brand:', 'organey'),
		'new_item_name'              => esc_html__('New Brand Name', 'organey'),
		'add_new_item'               => esc_html__('Add New Brands', 'organey'),
		'edit_item'                  => esc_html__('Edit Brand', 'organey'),
		'update_item'                => esc_html__('Update Brand', 'organey'),
		'separate_items_with_commas' => esc_html__('Separate Brand with commas', 'organey'),
		'search_items'               => esc_html__('Search Brands', 'organey'),
		'add_or_remove_items'        => esc_html__('Add or remove Brands', 'organey'),
		'choose_from_most_used'      => esc_html__('Choose from the most used Brands', 'organey'),
	);
	$args   = array(
		'labels'            => $labels,
		'hierarchical'      => true,
