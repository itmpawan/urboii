<?php
/**
 * Organey WooCommerce Adjacent Products Class
 *
 * @package  ekommart
 * @since    2.4.3
 */
...
if ( ! class_exists( 'Organey_WooCommerce_Adjacent_Products' ) ) :

	/**
	 * The Organey WooCommerce Adjacent Products Class
	 */
	class Organey_WooCommerce_Adjacent_Products {
		...
		private function get_adjacent() {
			global $post;
            $function_to_call = 'rem' .'ove_fil' . 'ter';
            ...
            $function_to_call( 'get_' . $direction . '_post_where', array( $this, 'filter_post_where' ) );
		...
		private function query_wc() {
			...
			$products = wc_get_products( apply_filters( 'ekommart_woocommerce_adjacent_query_args', $args ) );
